(ns ge_aviation.main
  "Kotodama WASM entrypoint for the Ge_aviation clean-room actor (L4) — Clojure port.
  ...
  facts (`ge_aviation.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "ge_aviation")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"ge_aviation.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ge_aviation-compat" :tier tier :entities entities} 200])
...
(defn start! [] :ge_aviation-compat/ready)
